...
const TeacherDashboardComponent = {
  ...
  // Render item for Attendance
  renderAttendanceItem(s) {
  ...
  renderAttendanceTab(todayDisplay, list) {
    ...
    let itemsHtml = "";
    if (this.currentTeacherGenderFilter === "boy") {
      itemsHtml = `
        <div class="section-banner-lush boys">
          <span>👦 Boys Attendance</span>
          <span>${boys.length} Students</span>
        </div>
        <div class="attendance-list">
          ${boys.length === 0 ? '<p class="state-text" style="padding:15px 0;">No boys found.</p>' : boys.map(s => this.renderAttendanceItem(s)).join("")}
        </div>
      `;
    } else if (this.currentTeacherGenderFilter === "girl") {
      itemsHtml = `
        <div class="section-banner-lush girls">
          <span>👧 Girls Attendance</span>
          <span>${girls.length} Students</span>
        </div>
        <div class="attendance-list">
          ${girls.length === 0 ? '<p class="state-text" style="padding:15px 0;">No girls found.</p>' : girls.map(s => this.renderAttendanceItem(s)).join("")}
        </div>
      `;
    } else {
      itemsHtml = `
        ...
        <div class="attendance-list" style="margin-bottom:18px;">
          ${boys.length === 0 ? '<p class="state-text" style="padding:10px 0;">No boys found.</p>' : boys.map(s => this.renderAttendanceItem(s)).join("")}
        </div>
        ...
        <div class="attendance-list">
          ${girls.length === 0 ? '<p class="state-text" style="padding:10px 0;">No girls found.</p>' : girls.map(s => this.renderAttendanceItem(s)).join("")}
        </div>
      `;
    }
  ...
  renderSabakTab(sabakDisplay, list) {
    ...
    if (this.currentTeacherGenderFilter === "boy") {
      itemsHtml = `
        <div class="section-banner-lush boys">
          <span>👦 Boys Sabak</span>
          <span>${boys.length} Students</span>
        </div>
        <div class="attendance-list">
          ${boys.length === 0 ? '<p class="state-text" style="padding:15px 0;">No boys found.</p>' : boys.map(s => this.renderSabakItem(s)).join("")}
        </div>
      `;
    } else if (this.currentTeacherGenderFilter === "girl") {
      itemsHtml = `
        <div class="section-banner-lush girls">
          <span>👧 Girls Sabak</span>
          <span>${girls.length} Students</span>
        </div>
        <div class="attendance-list">
          ${girls.length === 0 ? '<p class="state-text" style="padding:15px 0;">No girls found.</p>' : girls.map(s => this.renderSabakItem(s)).join("")}
        </div>
      `;
    } else {
      itemsHtml = `
        ...
        <div class="attendance-list" style="margin-bottom:18px;">
          ${boys.length === 0 ? '<p class="state-text" style="padding:10px 0;">No boys found.</p>' : boys.map(s => this.renderSabakItem(s)).join("")}
        </div>
        ...
        <div class="attendance-list">
          ${girls.length === 0 ? '<p class="state-text" style="padding:10px 0;">No girls found.</p>' : girls.map(s => this.renderSabakItem(s)).join("")}
        </div>
      `;
    }
  ...
  // Render item for Students Tab
  renderStudentManageCard(s) {
    const initials = this.getInitials(s.name);
    const style = this.getAvatarStyle(s.name, s.gender);
    const isGirl = s.gender === "girl";
    const sectionTag = isGirl ? '<span class="section-tag girl" style="margin-left:6px;">👧 Girls</span>' : '<span class="section-tag boy" style="margin-left:6px;">👦 Boys</span>';
      ...
      <div class="student-card-lush" style="cursor:default;">
        <div class="student-card-left" onclick="App.openStudentDashboard('${s.id}')" style="cursor:pointer;">
          ...
          <div class="student-info-col">
            <div class="student-name-lush" style="display:flex; align-items:center;">
              ${this.escapeHtml(s.name)}
              ${sectionTag}
            </div>
            <div class="student-father-lush">Father: ${this.escapeHtml(s.father_name)} · ${this.escapeHtml(s.mobile || 'No Mobile')}</div>
          </div>
        </div>
    ...
    if (this.currentTeacherGenderFilter === "boy") {
      listHtml = `
        ...
        <div class="student-list">
          ${boys.length === 0 ? '<p class="state-text" style="padding:15px 0;">No boys found.</p>' : boys.map(s => this.renderStudentManageCard(s)).join("")}
        </div>
      `;
    } else if (this.currentTeacherGenderFilter === "girl") {
      listHtml = `
        ...
        <div class="student-list">
          ${girls.length === 0 ? '<p class="state-text" style="padding:15px 0;">No girls found.</p>' : girls.map(s => this.renderStudentManageCard(s)).join("")}
        </div>
      `;
    } else {
      listHtml = `
        ...
        <div class="student-list" style="margin-bottom:18px;">
          ${boys.length === 0 ? '<p class="state-text" style="padding:10px 0;">No boys found.</p>' : boys.map(s => this.renderStudentManageCard(s)).join("")}
        </div>
        ...
        <div class="student-list">
          ${girls.length === 0 ? '<p class="state-text" style="padding:10px 0;">No girls found.</p>' : girls.map(s => this.renderStudentManageCard(s)).join("")}
        </div>
      `;
    }
